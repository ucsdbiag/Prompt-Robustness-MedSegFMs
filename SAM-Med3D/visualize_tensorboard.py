...
def process_pair(aug_npz, orig_img_npz, orig_gt_npz, writer, tag_prefix):
    
    img = np.load(orig_img_npz, allow_pickle=True)['imgs']
    gts = np.load(orig_gt_npz, allow_pickle=True)['gts']
    segs = np.load(aug_npz, allow_pickle=True)['preds']
    prompt = np.load(aug_npz, allow_pickle=True)['prompts']
    img, gts, prompt = np.squeeze(img), np.squeeze(gts), np.squeeze(prompt)
...
def main():
    ...
    p.add_argument("--aug-dir", default="./preds", help="Directory containing augmented npz files")
    p.add_argument("--aug-prefix", help="dilate/erode/hole etc prefix under aug-dir")
    p.add_argument("--orig-prefix", default="./img_npz", help="Prefix to use for original filenames")
    p.add_argument("--out-logdir", default="./tensorboard", help="TensorBoard logdir output")
    p.add_argument("--ext", default="npz", help="File extension to search (default npz)")
    args = p.parse_args()

    organlist = os.listdir(args.orig_prefix)
    for organ in sorted(organlist):
        for radius in range(1,11):
            aug_dir = os.path.join(args.aug_dir, f"{organ}/{args.aug_prefix}/radius={radius}")
            log_dir = os.path.join(args.out_logdir, f"{organ}/{args.aug_prefix}/radius={radius}")
            if args.aug_prefix in ['no-aug', 'shift']:
                aug_dir = os.path.join(args.aug_dir, f"{organ}/{args.aug_prefix}/radius=None")
                log_dir = os.path.join(args.out_logdir, f"{organ}/{args.aug_prefix}")
            ...
            for aug_path in files:
                f = os.path.basename(aug_path) # XXXX_pred+prompt.npz
                orig_path_img = os.path.join(args.orig_prefix, organ, f.replace("pred+prompt", "img+gt"))
                # print("Processing:", aug_path, " -> original:", orig_path_img)
                process_pair(aug_path, orig_path_img, orig_path_img, writer, tag_prefix=f.split("_")[0])
