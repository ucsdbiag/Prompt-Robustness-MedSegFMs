...
if __name__ == "__main__":
    ...
    import sys
    dataset_root = "/playpen/soumitri/data"
    dataset_list = [sys.argv[1]] # "amos22"
    suffix = sys.argv[2] # Tr or Va
    ...
    target_dir = f"./data_preproc/{dataset_list[0]}"
    split = {"Tr":"training", "Va":"validation", "Ts":"test"}[suffix]  # or "training" for suffix="Tr"
    ...
    for dataset in dataset_list:
        ...
        resample_dir = osp.join(dataset_dir, "images" + suffix + "_1.5") 
        os.makedirs(resample_dir, exist_ok=True)
        for idx, cls_name in meta_info["labels"].items():
            cls_name = cls_name.replace(" ", "_")
            idx = int(idx)
            dataset_name = dataset.split("_", maxsplit=1)[1] if "_" in dataset else dataset
            target_cls_dir = osp.join(target_dir, cls_name, dataset_name)
            target_img_dir = osp.join(target_cls_dir, "images" + suffix)
            target_gt_dir = osp.join(target_cls_dir, "labels" + suffix)
            os.makedirs(target_img_dir, exist_ok=True)
            os.makedirs(target_gt_dir, exist_ok=True)
            for item in tqdm(meta_info[split], desc=f"{dataset_name}-{cls_name}"):
                img, gt = item["image"], item["label"]
                img = osp.join(dataset_dir, img.replace(".nii.gz", ".nii.gz"))
                ...
                target_img_path = osp.join(target_img_dir, osp.basename(img).replace(".nii.gz", ".nii.gz"))
                target_gt_path = osp.join(target_gt_dir, osp.basename(gt).replace(".nii.gz", ".nii.gz"))
