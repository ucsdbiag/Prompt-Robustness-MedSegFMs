...
#### click based on input logit map and no gt -- only using logit map uncertainty ####
def next_click_from_logits_uncertainty_no_gt(
    prev_logits : torch.Tensor,      # [B,1,D,H,W]
    gt_mask      : torch.Tensor = None,
    mode         : str = "uncertainty",  # or "simulate_errors"
):
    """
    mode="simulate_errors": use gt_mask to force FP/FN clicks
    mode="uncertainty": ignore gt and click where model is least certain
    """
    B = prev_logits.shape[0]
    probs = torch.sigmoid(prev_logits)  # [B,1,D,H,W]

    batch_points = []
    batch_labels = []

    if mode=="simulate_errors" and gt_mask is not None:
        # build FN/FP from threshold=0.5
        pred = probs > 0.5
        true = gt_mask>0
        fn =  true & (~pred)
        fp = (~true)&  pred
    ...
    for i in range(B):
        if mode=="simulate_errors" and gt_mask is not None:
            fn_pts = torch.argwhere(fn[i,0])
            fp_pts = torch.argwhere(fp[i,0])
            if len(fn_pts)==0 and len(fp_pts)==0:
                fallback = True
            else:
                fallback=False
                # sample from fn or fp
                if len(fn_pts)>0 and (len(fp_pts)==0 or np.random.rand()>0.5):
                    point = fn_pts[np.random.randint(len(fn_pts))]
                    is_pos = True
                else:
                    point = fp_pts[np.random.randint(len(fp_pts))]
                    is_pos = False

        else:  # uncertainty‐driven
            fallback=False
            un = torch.abs(probs[i,0] - 0.5)
            flat = un.view(-1)
            idx = torch.argmin(flat)  # most uncertain
            z,y,x = np.unravel_index(idx.cpu(), un.shape)
            point = torch.tensor([z,y,x], device=prev_logits.device)
            is_pos = (probs[i,0,z,y,x] > 0.5).item()

        if fallback:
            # pick truly most uncertain
            un = torch.abs(probs[i,0] - 0.5)
            flat = un.view(-1)
        ...
        # format to (1,1,3) and label (1,1)
        bp = point.clone().detach().view(1,1,-1)  # drop depth if 2D, or keep all dims
        bl = torch.tensor([[int(is_pos)]], device=prev_logits.device)
...
def get_next_click3D_torch(prev_seg, gt_semantic_seg, mask_threshold=None):

    if mask_threshold is None: mask_threshold = 0.5
